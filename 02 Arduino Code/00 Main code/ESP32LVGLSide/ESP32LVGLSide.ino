...
// member data (new format: Unix times + simpler visaType)
struct MemberInfo {
  String docPath;     // members/m1
  String name;
  String country;
...
// "MM/DD/YYYY" -> Unix time (sec)
int32_t mdyToUnix(const String &mdy) {
  int m = 0, d = 0, y = 0;
  int p1 = mdy.indexOf('/');
  int p2 = mdy.indexOf('/', p1 + 1);
  if (p1 <= 0 || p2 <= p1 + 1) return 0;

  m = mdy.substring(0, p1).toInt();
  d = mdy.substring(p1 + 1, p2).toInt();
  y = mdy.substring(p2 + 1).toInt();

  if (m < 1 || m > 12 || d < 1 || d > 31 || y < 1970) return 0;
...
// (For seeding) Firestore JSON type generate
// 입력: name, "MM/DD/YYYY" birth, country, visaType, canFinancial, tendency, nowEpoch
String buildMemberDocBody(
  const char* name,
  const char* birthMDY,         // "MM/DD/YYYY"
  const char* country,
  const char* visaType,         // "" or "F-1" 등
  bool canFinancialTransactions,
  int tendency0to10,
  int32_t nowEpoch              // lastBatteryReplacementDate 로 사용
) {
  int32_t birthUnix = mdyToUnix(String(birthMDY));
  int32_t batteryDueUnix = 0;
  if (birthUnix > 0) {
    batteryDueUnix = birthUnix + (int32_t)(365L * 24L * 3600L);  // 단순 1년(365일) 후
  }
  ...
  fields["name"]["stringValue"] = name;
  fields["birthDate"]["integerValue"] = String(birthUnix);
  fields["batteryDueDate"]["integerValue"] = String(batteryDueUnix);
  fields["lastBatteryReplacementDate"]["integerValue"] = String(nowEpoch);

  fields["country"]["stringValue"] = country;
  fields["visaType"]["stringValue"] = visaType;  // 빈 문자열이면 "비자 없음" 의미
...
void seedMembersOnce() {
  ...
  if (nowT < 0) nowT = 0;
  int32_t nowEpoch = (int32_t)nowT;

  String bodyM1 = buildMemberDocBody("Minkyu Kim", "10/25/1997", "South Korea",
                                     "F-1", true, 9, nowEpoch);
  String bodyM2 = buildMemberDocBody("Max Hahn", "03/15/2000", "USA",
                                     "", false, 5, nowEpoch);
  String bodyM3 = buildMemberDocBody("Augie Fesh", "06/23/1994", "Canada",
                                     "H-1B", true, 2, nowEpoch);
...
void drawMemberInfo(const MemberInfo &m) {
  ...
  gfx->setCursor(x, y + 62*size);
  gfx->print("Birth: ");
  gfx->print(unixToMDY(m.birthDateUnix));

  gfx->setCursor(x, y + 78*size);
  gfx->print("Battery Due: ");
  gfx->print(unixToMDY(m.batteryDueUnix));

  gfx->setCursor(x, y + 94*size);
  gfx->print("Last Replace: ");
  gfx->print(unixToMDY(m.lastBatteryUnix));

  gfx->setCursor(x, y + 110*size);
  gfx->print("Visa: ");
  if (m.visaType.length() > 0) {
    gfx->print(m.visaType);
  } else {
    gfx->print("NONE");
  }
  ...
  gfx->setCursor(x, y + 126*size);
  gfx->print("Finance: ");
  gfx->print(m.canFinancial ? "YES" : "NO");

  gfx->setCursor(x, y + 142*size);
  gfx->print("Tendency: ");
  gfx->print(m.tendency);
  gfx->print("/10");
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (firebaseSignIn()) {
      ...
      if (!seeded || !existM1) {
        ...
        // 이미 존재해도 현재 스키마로 덮어쓰고 싶으면 아래 줄 유지
        seedMembersOnce();
        Serial.println("[SEED] already");
      }
      prefs.end();
